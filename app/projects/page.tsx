...
export default function ProjectsPage() {
  ...
  // Read ?butSkill=X (and optional ?track=) on first mount to pre-apply
  ...
  useEffect(() => {
    ...
    if (trackParam === "pro" || trackParam === "school") {
  ...
  const filters: { key: Category; label: string }[] = [
    ...
    { key: "web", label: lang === "fr" ? "Développement web" : "Web Development" },
    { key: "uxui", label: "UX/UI" },
    { key: "visuels", label: lang === "fr" ? "Productions visuelles" : "Visual productions" },
  ...
  const filtered = projectsData.filter((p) => {
    ...
    // BUT skill filter — applies in all tracks now (since track Académique ≈ Tout)
    if (butSkill !== "tous") {
  ...
  // When switching track, keep the skill filter (since competences apply across)
  ...
  return (
    <div className="min-h-screen pt-24 pb-20 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Track switcher (Tout / Pro & Freelance) */}
        <div
          className="inline-flex p-1 mb-6 rounded-full bg-text-primary/5 dark:bg-white/5 border border-text-primary/15 dark:border-white/10"
          role="tablist"
        ...
        >
          {(["all", "pro"] as Track[]).map((k) => (
            <button
              ...
              className={`font-body text-sm px-4 py-1.5 rounded-full transition-all duration-200 inline-flex items-center gap-1.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-text-secondary dark:focus-visible:ring-pink ${
                track === k
                  ? "bg-text-secondary dark:bg-pink text-white shadow-sm"
        ...
        {/* BUT competence filters — always visible: pluridisciplinarité across all tracks */}
        <div className="flex flex-wrap items-center gap-2 mb-10 pl-1">
          <span className="font-bebas text-[10px] text-text-secondary dark:text-white/55 uppercase tracking-widest mr-1">
          ...
          {skillsRow.map((skill) => (
            <button
              ...
              className={`font-bebas text-xs px-3 py-1.5 rounded-full border transition-all duration-200 capitalize focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-text-secondary dark:focus-visible:ring-pink ${
                butSkill === skill
                  ? "bg-text-secondary dark:bg-pink border-text-secondary dark:border-pink text-white"
                  : "border-text-primary/20 dark:border-white/15 text-text-secondary dark:text-white/65 hover:border-text-primary/40 dark:hover:border-white/30 hover:text-text-primary dark:hover:text-white"
              }`}
            >
              {skill}
            </button>
